...
import           Cardano.SMASH.Types (ApiResult, HealthStatus, PolicyResult, PoolFetchError,
                   PoolIdent (..), PoolMetaHash, PoolMetadataRaw, SmashURL, TickerName,
...
type OfflineMetadataAPI = "api" :> APIVersion :> "metadata" :> Capture "id" PoolIdent :> Capture "hash" PoolMetaHash :> Get '[JSON] (Headers '[Header "Cache-Control" Text] (ApiResult DBFail PoolMetadataRaw))
...
type AddPoolAPI = "api" :> APIVersion :> "metadata" :> Capture "id" PoolIdent :> Capture "hash" PoolMetaHash :> ReqBody '[OctetStream] PoolMetadataRaw :> ApiRes Post PoolIdent
